...
![](../data/logoET.png){fig-align="center"}
...
We will start by discussing the problem of _detecting_ a gravitational signal,
and then move to _analyzing_ it.

## Gravitational wave detection

The gravitational-wave **detection** problem statement is as follows: we have data 
...
The fundamental strategy used in order to detect gravitational wave signal is called _matched filtering_. 
Consider a stream of data $d(t)$ consisting of a signal $h(t)$ plus additive Gaussian noise $n(t)$ described by a Power Spectral Density $S_n(f)$, such that $d(t) = n(t) + h(t)$.

We want to detect $h$ in a situation where the absolute value of $h(t)$ is much smaller than that of $n(t)$. A _linear filter_ can be used to this end: we will extract some function of the data in the form

$$ \hat{\rho}(\tau ) = \int d(t + \tau) f(t) \text{d}t
$$

for some $f(t)$, such that this $\hat{\rho}$ peaks where the signal is, in as promiment a way as possible.
Formally: we want the ratio 

$$ \frac{S}{N} = \frac{\hat{\rho} (\text{a signal is present})}{ \text{root-mean-square of } \hat{\rho}}
$$

to be as large as possible.
It turns out that the best way to accomplish this is through a filter that's weighted with the noise spectrum.

Let us define $(\cdot | \cdot)$ as the noise-weighed inner product between waveforms,  

$$ (a | b) = 4 \Re \int_0^\infty \frac{a(f) b^*(f)}{S_n(f)}\text{d}f \,,
$$

The optimal statistic $\hat{\rho}$ we want will then be (proportional to) $(d|h)$.
Then, the signal-to-noise ratio will be:

$$ \rho = \frac{\hat{\rho}}{\hat{\rho}_{\text{rms}}} = \frac{S}{N} = \frac{(d|h)}{(h|h)}
$$
...
we cannot know in advance what the noise realization will be. Fortunately, the expected contribution
of the term $(n|h)$ is zero since $\langle n(t) \rangle = 0$, so the expectation value is just $\rho \approx \sqrt{(h|h)}$: 
...
specifics of the waveform model: for one, it discards all phase information, since the waveform appears in it as $|h(f)|^2$.
...
Here is a graphical representation of correlation: the distribution for a multivariate normal with correlation matrix 
...
parameters. Here is an example showing the correlation between distance and inclination angle for GW170817.
...
These are different, as the figure in the [Correlations](#correlations) section shows.
