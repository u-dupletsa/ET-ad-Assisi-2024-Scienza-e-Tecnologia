...
We want to "find" $h$ in a situation where the absolute value of $h(t)$ is much smaller than that of $n(t)$. A _linear filter_ can be used to this end: we will extract some function of the data in the form
...
$$ \frac{S}{N} = \frac{\hat{\rho} (\text{a signal is present})}{\hat{\rho} (\text{root-mean-square})}
$$
...
A commonly-used way to normalize it, then, is to use 
...
cosi = f['IMRPhenomPv2NRT_lowSpin_posterior']['costheta_jn']
dist = f['IMRPhenomPv2NRT_lowSpin_posterior']['luminosity_distance_Mpc']
